...
class SampleDataService {
    static func createSampleData(in context: ModelContext) {
        ...
        let max      = person("demo-max",    isFR ? "Jean Dupont"         : isES ? "Carlos García"      : isEN ? "James Wilson"       : "Max Mustermann",        bday(0,   32), isFR ? "Ami"         : isES ? "Amigo"     : isEN ? "Friend"    : "Freund")
        let erika    = person("demo-erika",  isFR ? "Marie Martin"        : isES ? "María López"        : isEN ? "Emily Parker"       : "Erika Musterfrau",      bday(2,   38), isFR ? "Sœur"        : isES ? "Hermana"   : isEN ? "Sister"    : "Schwester")
        let hans     = person("demo-hans",   isFR ? "Pierre Bernard"      : isES ? "Pedro Martínez"     : isEN ? "David Thompson"     : "Hans Beispiel",         bday(5,   29), isFR ? "Collègue"    : isES ? "Colega"    : isEN ? "Colleague" : "Kollege")
        let anna     = person("demo-anna",   isFR ? "Isabelle Lefebvre"   : isES ? "Ana Rodríguez"      : isEN ? "Margaret Johnson"   : "Anna Muster",           bday(7,   45), isFR ? "Mère"        : isES ? "Madre"     : isEN ? "Mother"    : "Mutter")
        let peter    = person("demo-peter",  isFR ? "Louis Moreau"        : isES ? "Luis Hernández"     : isEN ? "Robert Davis"       : "Peter Beispielmann",    bday(12,  26), isFR ? "Frère"       : isES ? "Hermano"   : isEN ? "Brother"   : "Bruder")

        // --- Diese Woche / diesen Monat ---
        let lisa     = person("demo-lisa",   isFR ? "Camille Petit"       : isES ? "Sofia Torres"       : isEN ? "Lisa Martinez"      : "Lisa Testmann",         bday(15,  41), isFR ? "Collègue"    : isES ? "Colega"    : isEN ? "Colleague" : "Kollegin")
        let thomas   = person("demo-thomas", isFR ? "Thomas Leroy"        : isES ? "Miguel Fernández"   : isEN ? "Thomas Brown"       : "Thomas Muster",         bday(18,  34), isFR ? "Ami"         : isES ? "Amigo"     : isEN ? "Friend"    : "Freund")
        let julia    = person("demo-julia",  isFR ? "Julie Roux"          : isES ? "Julia Sánchez"      : isEN ? "Julia Anderson"     : "Julia Musterfrau",      bday(22,  27), isFR ? "Amie"        : isES ? "Amiga"     : isEN ? "Friend"    : "Freundin")
        let sophie   = person("demo-sophie", isFR ? "Sophie Dubois"       : isES ? "Carmen Díaz"        : isEN ? "Sophie Miller"      : "Sophie Beispiel",       bday(25,  31), isFR ? "Partenaire"  : isES ? "Pareja"    : isEN ? "Partner"   : "Partnerin")
        let michael  = person("demo-michael",isFR ? "Michel Garnier"      : isES ? "Antonio García"     : isEN ? "Michael Taylor"     : "Michael Normalverb.",   bday(28,  50), isFR ? "Oncle"       : isES ? "Tío"       : isEN ? "Uncle"     : "Onkel")

        // --- Nächste 2 Monate ---
        let maria    = person("demo-maria",  isFR ? "Martine Girard"      : isES ? "Elena Moreno"       : isEN ? "Maria Garcia"       : "Maria Beispiel",        bday(35,  58), isFR ? "Tante"       : isES ? "Tía"       : isEN ? "Aunt"      : "Tante")
        let felix    = person("demo-felix",  isFR ? "Félix Mercier"       : isES ? "Diego López"        : isEN ? "Felix Harris"       : "Felix Musterknabe",     bday(42,  23), isFR ? "Cousin"      : isES ? "Primo"     : isEN ? "Cousin"    : "Cousin")
        let klara    = person("demo-klara",  isFR ? "Claire Fontaine"     : isES ? "Clara Jiménez"      : isEN ? "Clara Robinson"     : "Klara Beispielfrau",    bday(48,  36), isFR ? "Amie"        : isES ? "Amiga"     : isEN ? "Friend"    : "Freundin")
        let otto     = person("demo-otto",   isFR ? "René Chevalier"      : isES ? "Roberto Torres"     : isEN ? "Richard Moore"      : "Otto Normalverbraucher",bday(55,  52), isFR ? "Père"        : isES ? "Padre"     : isEN ? "Father"    : "Vater")
        let emma     = person("demo-emma",   isFR ? "Emma Laurent"        : isES ? "Lucía García"       : isEN ? "Emma Wilson"        : "Emma Musterkind",       bday(62,   8), isFR ? "Nièce"       : isES ? "Sobrina"   : isEN ? "Niece"     : "Nichte")
        let paul     = person("demo-paul",   isFR ? "Paul Simon"          : isES ? "Pablo Martínez"     : isEN ? "Paul Walker"        : "Paul Muster",           bday(68,  17), isFR ? "Neveu"       : isES ? "Sobrino"   : isEN ? "Nephew"    : "Neffe")
        let kurt     = person("demo-kurt",   isFR ? "Marc Perrin"         : isES ? "Javier Ruiz"        : isEN ? "Kurt Reynolds"      : "Kurt Musterboss",       bday(75,  44), isFR ? "Chef"        : isES ? "Jefe"      : isEN ? "Boss"      : "Chef")
        let sarah    = person("demo-sarah",  isFR ? "Sarah Rousseau"      : isES ? "Sara Gómez"         : isEN ? "Sarah Collins"      : "Sarah Beispielfrau",    bday(82,  33), isFR ? "Collègue"    : isES ? "Colega"    : isEN ? "Colleague" : "Kollegin")
        let markus   = person("demo-markus", isFR ? "Marc Blanc"          : isES ? "Marcos Pérez"       : isEN ? "Marcus Reed"        : "Markus Testmann",       bday(88,  28), isFR ? "Ami"         : isES ? "Amigo"     : isEN ? "Friend"    : "Freund")
        let ingrid   = person("demo-ingrid", isFR ? "Françoise Dupuy"     : isES ? "Isabel Fernández"   : isEN ? "Dorothy Evans"      : "Ingrid Muster",         bday(95,  67), isFR ? "Grand-mère"  : isES ? "Abuela"    : isEN ? "Grandma"   : "Oma")

        // --- 3–6 Monate ---
        let gerhard  = person("demo-gerhard",isFR ? "Georges Morel"       : isES ? "Gerardo López"      : isEN ? "George Campbell"    : "Gerhard Beispiel",      bday(102, 70), isFR ? "Grand-père"  : isES ? "Abuelo"    : isEN ? "Grandpa"   : "Opa")
        let nina     = person("demo-nina",   isFR ? "Nina Aubert"         : isES ? "Nina Vargas"        : isEN ? "Nina Stewart"       : "Nina Musterfee",        bday(108, 22), isFR ? "Cousine"     : isES ? "Prima"     : isEN ? "Cousin"    : "Cousine")
        let stefan   = person("demo-stefan", isFR ? "Stéphane Colin"      : isES ? "Esteban Castro"     : isEN ? "Steven Cooper"      : "Stefan Beispielm.",     bday(115, 39), isFR ? "Frère"       : isES ? "Hermano"   : isEN ? "Brother"   : "Bruder")
        let lena     = person("demo-lena",   isFR ? "Léa Bertrand"        : isES ? "Valentina Ramos"    : isEN ? "Lena Brooks"        : "Lena Musterkind",       bday(120, 11), isFR ? "Nièce"       : isES ? "Sobrina"   : isEN ? "Niece"     : "Nichte")
        let tobias   = person("demo-tobias", isFR ? "Thibault Renard"     : isES ? "Tomás Herrera"      : isEN ? "Toby Richardson"    : "Tobias Normalbürger",   bday(128, 25), isFR ? "Collègue"    : isES ? "Colega"    : isEN ? "Colleague" : "Kollege")
        let petra    = person("demo-petra",  isFR ? "Patricia Vincent"    : isES ? "Patricia Vega"      : isEN ? "Patricia Wood"      : "Petra Beispielfrau",    bday(135, 48), isFR ? "Tante"       : isES ? "Tía"       : isEN ? "Aunt"      : "Tante")
        let daniel   = person("demo-daniel", isFR ? "Daniel Muller"       : isES ? "Daniel Ortega"      : isEN ? "Daniel Foster"      : "Daniel Muster",         bday(142, 30), isFR ? "Ami"         : isES ? "Amigo"     : isEN ? "Friend"    : "Freund")
        let monika   = person("demo-monika", isFR ? "Monique Faure"       : isES ? "Mónica Reyes"       : isEN ? "Monica Bell"        : "Monika Beispiel",       bday(149, 55), isFR ? "Mère"        : isES ? "Madre"     : isEN ? "Mother"    : "Mutter")
        let jan      = person("demo-jan",    isFR ? "Julien Legrand"      : isES ? "Juan Delgado"       : isEN ? "Jack Howard"        : "Jan Testmann",          bday(155, 21), isFR ? "Neveu"       : isES ? "Sobrino"   : isEN ? "Nephew"    : "Neffe")
        let laura    = person("demo-laura",  isFR ? "Laura Bonnet"        : isES ? "Laura Romero"       : isEN ? "Laura Bennett"      : "Laura Musterfrau",      bday(162, 26), isFR ? "Amie"        : isES ? "Amiga"     : isEN ? "Friend"    : "Freundin")

        // --- 6–12 Monate ---
        let bernhard = person("demo-bernhard",isFR ? "Bernard Lacroix"   : isES ? "Bernardo Cruz"     : isEN ? "Bernard Gray"      : "Bernhard Muster",       bday(168, 43), isFR ? "Oncle"       : isES ? "Tío"       : isEN ? "Uncle"     : "Onkel")
        let karin    = person("demo-karin",  isFR ? "Karine Masson"       : isES ? "Carolina Flores"    : isEN ? "Karen Price"        : "Karin Beispielfrau",    bday(175, 38), isFR ? "Collègue"    : isES ? "Colega"    : isEN ? "Colleague" : "Kollegin")
        let florian  = person("demo-florian",isFR ? "Florian Bourgeois"   : isES ? "Florián Mendoza"    : isEN ? "Florian Ross"       : "Florian Normalv.",      bday(182, 19), isFR ? "Cousin"      : isES ? "Primo"     : isEN ? "Cousin"    : "Cousin")
        let ursula   = person("demo-ursula", isFR ? "Ursule Guerin"       : isES ? "Úrsula Morales"     : isEN ? "Ruth Morgan"        : "Ursula Musterdame",     bday(188, 72), isFR ? "Grand-mère"  : isES ? "Abuela"    : isEN ? "Grandma"   : "Oma")
        let dominik  = person("demo-dominik",isFR ? "Dominique Olivier"   : isES ? "Dominik Guzmán"     : isEN ? "Dominic Hayes"      : "Dominik Beispiel",      bday(195, 24), isFR ? "Ami"         : isES ? "Amigo"     : isEN ? "Friend"    : "Freund")
        let helga    = person("demo-helga",  isFR ? "Hélène Clement"      : isES ? "Helena Castillo"    : isEN ? "Helen Peterson"     : "Helga Musterdame",      bday(200, 65), isFR ? "Tante"       : isES ? "Tía"       : isEN ? "Aunt"      : "Tante")
        let patrick  = person("demo-patrick",isFR ? "Patrick Gauthier"    : isES ? "Patricio Ibáñez"    : isEN ? "Patrick Kelly"      : "Patrick Testperson",    bday(208, 31), isFR ? "Collègue"    : isES ? "Colega"    : isEN ? "Colleague" : "Kollege")
        let andrea   = person("demo-andrea", isFR ? "Andrée Leroux"       : isES ? "Andrea Silva"       : isEN ? "Andrea Mitchell"    : "Andrea Musterfrau",     bday(215, 46), isFR ? "Cheffe"      : isES ? "Jefa"      : isEN ? "Boss"      : "Chefin")
        let tim      = person("demo-tim",    isFR ? "Tim Barbier"         : isES ? "Timoteo Varela"     : isEN ? "Tim Sanders"        : "Tim Beispielkind",      bday(222, 14), isFR ? "Neveu"       : isES ? "Sobrino"   : isEN ? "Nephew"    : "Neffe")
        let sandra   = person("demo-sandra", isFR ? "Sandra Arnaud"       : isES ? "Sandra Ríos"        : isEN ? "Sandra Cox"         : "Sandra Musterperson",   bday(228, 29), isFR ? "Amie"        : isES ? "Amiga"     : isEN ? "Friend"    : "Freundin")
        let christian = person("demo-christian",isFR ? "Christian Giraud" : isES ? "Cristian Navarro" : isEN ? "Chris Murphy"    : "Christian Muster",       bday(235, 37), isFR ? "Frère"       : isES ? "Hermano"   : isEN ? "Brother"   : "Bruder")
        let brigitte = person("demo-brigitte",isFR ? "Brigitte Perrot"    : isES ? "Brigida Santos"    : isEN ? "Barbara Russell"   : "Brigitte Beispiel",     bday(242, 61), isFR ? "Grand-mère"  : isES ? "Abuela"    : isEN ? "Grandma"   : "Oma")
        let oliver   = person("demo-oliver", isFR ? "Olivier Caron"       : isES ? "Oliver Serrano"     : isEN ? "Oliver Grant"       : "Oliver Normalverb.",    bday(248, 27), isFR ? "Ami"         : isES ? "Amigo"     : isEN ? "Friend"    : "Freund")
        let katrin   = person("demo-katrin", isFR ? "Catherine Noel"      : isES ? "Catalina León"      : isEN ? "Katherine Lee"      : "Katrin Musterfrau",     bday(255, 34), isFR ? "Collègue"    : isES ? "Colega"    : isEN ? "Colleague" : "Kollegin")
        let werner   = person("demo-werner", isFR ? "Werner Vidal"        : isES ? "Wenceslao Vera"     : isEN ? "Walter Scott"       : "Werner Beispielm.",     bday(260, 58), isFR ? "Oncle"       : isES ? "Tío"       : isEN ? "Uncle"     : "Onkel")
        let melanie  = person("demo-melanie",isFR ? "Mélanie Schmitt"     : isES ? "Melania Blanco"     : isEN ? "Melanie Young"      : "Melanie Musterperson",  bday(268, 23), isFR ? "Cousine"     : isES ? "Prima"     : isEN ? "Cousin"    : "Cousine")
        let hubert   = person("demo-hubert", isFR ? "Hubert Marchand"     : isES ? "Huberto Aguilar"    : isEN ? "Herbert King"       : "Hubert Normalverb.",    bday(275, 74), isFR ? "Grand-père"  : isES ? "Abuelo"    : isEN ? "Grandpa"   : "Opa")
        let tanja    = person("demo-tanja",  isFR ? "Tania Lemaire"       : isES ? "Tania Molina"       : isEN ? "Tanya Adams"        : "Tanja Beispielfrau",    bday(280, 41), isFR ? "Amie"        : isES ? "Amiga"     : isEN ? "Friend"    : "Freundin")
        let Sebastian = person("demo-sebastian",isFR ? "Sébastien Brunet" : isES ? "Sebastián Parra"  : isEN ? "Sebastian Clark" : "Sebastian Muster",       bday(290, 33), isFR ? "Ami"         : isES ? "Amigo"     : isEN ? "Friend"    : "Freund")
        let irene    = person("demo-irene",  isFR ? "Irène Dumont"        : isES ? "Irene Cabrera"      : isEN ? "Irene Wright"       : "Irene Musterdame",      bday(350, 69), isFR ? "Tante"       : isES ? "Tía"       : isEN ? "Aunt"      : "Tante")

        // Test: Geburtstag gestern + gekauftes Geschenk → Auto-Transition testen
        let gestern  = person("demo-gestern",isFR ? "Hier Anniversaire"  : isES ? "Ayer Cumpleaños"    : isEN ? "Yesterday Birthday" : "Gestern Geburtstag",   bday(-1, 30), isFR ? "Ami" : isES ? "Amigo" : isEN ? "Friend" : "Freund")
